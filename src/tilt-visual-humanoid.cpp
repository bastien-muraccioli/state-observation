...
ObserverBase::StateVector TiltVisualHumanoid::oneStepEstimation_()
{
  ...
  Matrix3 dR_hat = Orientation::zeroRotation();

  Matrix3 rot_diff = R_hat_.toMatrix3().transpose() * yR.toMatrix3();

  Vector3 sigma = rho1_ * (R_hat_.toMatrix3().transpose() * Vector3::UnitZ()).cross(x2_hat_prime_)
                  + rho2_ * kine::skewSymmetricToRotationVector(rot_diff - rot_diff.transpose());

  dR_hat = R_hat_.toMatrix3() * kine::skewSymmetric(yg - sigma);

  Vector3 dt_omega = kine::skewSymmetricToRotationVector(dR_hat * R_hat_.toMatrix3()) * dt_;
